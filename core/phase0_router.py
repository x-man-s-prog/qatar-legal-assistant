...
_SELF_PATTERNS: tuple[tuple[str, str], ...] = (
    (
        ...
        "• البحث في أحكام محكمة التمييز (11,462 حكم).\n"
        "• حساب المستحقات (نهاية خدمة، تعويض فصل).\n"
        "• الاستشارات القانونية بناءً على 49,048 نص قانوني.\n\n"
        "تفضل بسؤالك!"
    ),
    (
        ...
        "• **البحث القانوني:** 11,462 حكم تمييز + 49,048 نص قانوني.\n"
        "• **الحاسبات:** مكافأة نهاية الخدمة، تعويض الفصل التعسفي.\n"
        "• **الجداول المرجعية:** جدول المخدرات، المخالفات المرورية، وغيرها.\n\n"
        "تفضل بسؤالك!"
    ),
    (
        r"كم\s+(?:مبدأ|حكم|مادة|نص|قانون|تشريع|عدد)\s+(?:\w+\s+){0,3}?(?:عندك|لديك|في\s+قاعدة)"
        r"|كم\s+عدد\s+(?:ال)?(?:قوانين|التشريعات|النصوص|المواد|الأحكام|المبادئ)"
        r"|حجم\s+قاعدة\s+البيانات"
        r"|وش\s+عندك\s+(?:من|في)\s+(?:قوانين|بيانات)",
        "قاعدة بياناتي تحتوي على:\n"
        "• **49,048 نص قانوني قطري نشط** — جميع القوانين والتشريعات "
        "المنشورة على موقع الميزان.\n"
        "• **11,462 حكم من أحكام محكمة التمييز** — مفصّلة بالكامل.\n"
        "• **663 مبدأ قانوني** مستخلص من أحكام التمييز.\n"
        "• قوالب مذكرات قانونية جاهزة.\n"
        "• جداول مرجعية: المخدرات، المخالفات المرورية، الرواتب، "
        "العقوبات.\n\n"
        "أستطيع البحث الدقيق في كل هذه النصوص + إصدار أحكام مبنية "
        "على مواد قانونية محددة. تبي أبحث لك في موضوع معيّن؟"
    ),
    (
        r"(?:عندك|لديك)\s+قاعدة\s+بيانات"
        r"|هل\s+عندك\s+(?:نصوص|مواد|قوانين)"
        r"|(?:هل\s+)?تستطيع\s+البحث",
        "نعم، قاعدة بياناتي غنية ومتجددة:\n"
        "• **49,048 نص قانوني قطري نشط** (قوانين، تشريعات، لوائح).\n"
        "• **11,462 حكم تمييز** مع المبادئ القضائية.\n"
        "• أستطيع جلب نص أي مادة مباشرة من قاعدة البيانات.\n"
        "• أستطيع البحث بـ hybrid search (vector + keyword + full-text).\n\n"
        "تفضّل بسؤالك."
    ),
)
...
def check_self_info(query: str) -> Optional[str]:
    ...
    # Increased from 10 to 15 words so "كم مبدأ قضائي عندك في قاعدة البيانات"
    # (6 words) and similar phrasings still match even with connectors.
...
# 7. MEMO REQUEST
# ─────────────────────────────────────────────────────────────────────
# ⚠ MEMO DETECTION SOURCE-OF-TRUTH GUARDRAIL ⚠
# Memo-trigger knowledge is currently split across THREE files. Any
# change to memo phrasing must be mirrored in all three places or the
# behavior will drift:
#   1. core/phase0_router.py     :: _MEMO_TRIGGERS   ← (this file)
#   2. routers/query_router.py   :: memo_continuation block in
#                                    query_stream (A/B/C triggers) +
#                                    _MEMO_TOPIC_MAP + _MEMO_GAPS
#   3. core/runtime_v2/pipeline.py :: _DRAFT_TRIGGERS
# TODO(cleanup P2): collapse all three into a single DomainRules field
# so a new memo phrasing is added in ONE place only.
# ═════════════════════════════════════════════════════════════════════
...
_REVIEW_PATTERNS = (
    # مراجعة المذكرات (الأصلية)
    r"راجع\s+(?:لي\s+)?(?:ال)?مذكر[ةه]",
    r"قيّ?م\s+(?:لي\s+)?(?:ال)?مذكر[ةه]",
    r"ب?أرسل\s+لك\s+مذكر[ةه]",
    r"برسل\s+لك\s+مذكر[ةه]",
    r"عدل\s+(?:لي\s+)?(?:ال)?مذكر[ةه]",
    # مراجعة العقود والاتفاقيات والمستندات (مضاف في P1 cleanup)
    r"راجع\s+(?:لي\s+)?(?:هذا\s+)?(?:ال)?(?:عقد|اتفاقي[ةه]|مستند|وثيق[ةه]|شرط|بند)",
    r"قيّ?م\s+(?:لي\s+)?(?:هذا\s+)?(?:ال)?(?:عقد|اتفاقي[ةه]|مستند|وثيق[ةه])",
    r"عدّ?ل\s+(?:لي\s+)?(?:هذا\s+)?(?:ال)?(?:عقد|اتفاقي[ةه]|بند)",
    r"حلّ?ل\s+(?:لي\s+)?(?:هذا\s+)?(?:ال)?(?:عقد|اتفاقي[ةه]|مستند)",
    r"اقرأ\s+(?:لي\s+)?(?:هذا\s+)?(?:ال)?(?:عقد|اتفاقي[ةه])",
    r"افحص\s+(?:لي\s+)?(?:هذا\s+)?(?:ال)?(?:عقد|اتفاقي[ةه]|بند)",
)
