...
# 6. Criminal — Drug Possession / Use (تعاطي وحيازة مخدرات)
_DRUG_USE = DomainExpertise(
    domain_key   = "criminal_drug_use",
    label        = "جرائم المخدرات — التعاطي والحيازة",
    common_legal_grounds = (
        LegalGroundExpertise(
            label              = "الدفاع في جريمة التعاطي (المادة 39)",
            primary_articles   = ("39", "41"),
            required_elements  = (
                "حيازة مادة مخدرة (الركن المادي)",
                "نية التعاطي (القصد الجنائي — الركن المعنوي)",
                "علم المتهم بطبيعة المادة",
                "عدم وجود إذن قانوني للحيازة (للأطباء مثلاً)",
            ),
            common_defenses    = (
                "انتفاء القصد الجنائي — المادة وُضعت دون علم",
                "بطلان إذن التفتيش — تفتيش بدون مسوّغ قانوني",
                "عدم وجود تحريات جدية سابقة للتفتيش",
                "المادة المضبوطة تحليلها يُشكك في طبيعتها",
                "الضبط دون شهود تفتيش (عدم انتظام)",
                "اعتراف المتهم تحت إكراه — باطل",
                "المتهم مريض — التعاطي علاج طبي بإذن",
            ),
            rebuttal_strategies = (
                "طلب تحليل مادة الضبط مجدداً عبر خبير محايد",
                "الطعن في محضر الضبط لعدم انتظامه",
                "طلب سماع الشهود الذين حضروا الضبط",
                "تقديم تقارير طبية إذا كان المتهم تحت علاج",
            ),
        ),
        LegalGroundExpertise(
            label              = "طلب الإعفاء من العقاب (التبرع بالعلاج)",
            primary_articles   = ("46", "47"),
            required_elements  = (
                "المتهم متعاطٍ (ليس متّجراً)",
                "التقدم للعلاج قبل اكتشاف الجريمة أو قبل المحاكمة",
                "عدم التوقف عن العلاج قبل إتمامه",
            ),
            common_defenses    = (
                "المتهم ليس متعاطياً بل متّجراً (وصف التهمة مختلف)",
                "العلاج بعد اكتشاف الجريمة — لا يصلح للإعفاء",
            ),
            rebuttal_strategies = (
                "تقديم ما يُثبت بدء العلاج قبل الاكتشاف",
                "شهادات المستشفى بمراحل العلاج",
            ),
        ),
    ),
    typical_counter_arguments = (
        "النيابة تدعي أن الضبط تم بناءً على إذن النيابة وتحريات جدية",
        "التحليل المخبري الرسمي يُثبت طبيعة المادة",
        "اعتراف المتهم في محضر الشرطة (إن وُجد) — يُطعن بالإكراه",
        "الكمية تدل على التعاطي لا الاتجار (للدفاع — قد تفيد)",
    ),
    required_evidence = (
        "محضر الضبط والتفتيش",
        "إذن النيابة بالتفتيش (أو ما يُشير إلى بطلانه)",
        "تقرير التحليل المخبري الرسمي",
        "شهادة الضباط الذين قاموا بالضبط",
        "شهود التفتيش (إن وُجدوا)",
        "ما يُثبت طبيعة التعاطي (طبية/استخدام شخصي) مقابل الاتجار",
    ),
    competent_court    = "المحكمة الجزائية — دائرة المخدرات",
    appeal_court       = "محكمة الاستئناف — الدائرة الجزائية",
    statute_of_limitations = (
        "جنحة (التعاطي والحيازة للتعاطي الشخصي) — تتقادم بثلاث سنوات. "
        "جناية (الاتجار) — تتقادم بعشر سنوات."
    ),
    required_documents = (
        "محضر الضبط",
        "إذن التفتيش",
        "تقرير التحليل",
        "سجل المتهم السابق (إن وُجد — للسوابق)",
        "تقارير طبية للدفاع",
    ),
    core_articles       = ("39", "41", "46", "47", "57"),
    landmark_principles = (
        "اليقين في المحاكمات الجنائية شرط — الشك يُفسّر لصالح المتهم",
        "إذن التفتيش يجب أن يكون مسبوقاً بتحريات جدية (مبدأ راسخ)",
        "الاعتراف تحت إكراه باطل ولا يُعتدّ به",
        "التبرع بالعلاج قبل الاكتشاف يُعفي من العقاب (المادة 46)",
        "المادة 57 من قانون العقوبات تُشدد العقوبة في حالة العود",
        "الكمية المضبوطة قرينة على القصد (اتجار/تعاطٍ)",
    ),
    procedural_notes = (
        "الحبس الاحتياطي شائع في قضايا المخدرات",
        "يحق للمتهم حضور محاميه من لحظة التحقيق",
        "التحقيق الابتدائي أمام النيابة العامة قبل الإحالة",
        "الاستئناف خلال 30 يوماً من صدور الحكم",
        "التمييز خلال 60 يوماً من صدور حكم الاستئناف",
    ),
    common_mistakes = (
        "الاعتراف بالحيازة للتعاطي ثم إنكار النية — تناقض يُضعف الدفاع",
        "عدم طلب نسخة من إذن التفتيش لفحص صحته",
        "إهمال شرط التبرع بالعلاج قبل الاكتشاف في حالة إثبات التعاطي",
        "الخلط بين جريمة التعاطي وجريمة الاتجار — عقوبات مختلفة تماماً",
    ),
)


# 7. Traffic — General Traffic Penalties (عقوبات المرور)
_TRAFFIC = DomainExpertise(
    domain_key   = "traffic",
    label        = "المخالفات والعقوبات المرورية القطرية",
    common_legal_grounds = (
        LegalGroundExpertise(
            label              = "الطعن في سحب رخصة القيادة",
            primary_articles   = ("19", "66"),      # قانون المرور رقم 19/2007
            required_elements  = (
                "صدور قرار إداري بسحب الرخصة",
                "تبليغ السائق بالقرار",
                "تقديم التظلم خلال المهلة القانونية",
            ),
            common_defenses    = (
                "عدم قانونية قرار السحب — مخالفة غير ثابتة",
                "السحب لفترة أطول من المقرر قانوناً",
                "عدم اتباع الإجراءات الإدارية الصحيحة",
                "تعارض مع قرار قضائي سابق",
            ),
            rebuttal_strategies = (
                "تقديم محاضر تفيد عدم ارتكاب المخالفة",
                "الاستشهاد بقرارات إدارية مماثلة تم إلغاؤها",
                "إثبات الضرر الحاصل من استمرار السحب",
            ),
        ),
        LegalGroundExpertise(
            label              = "التعويض عن الضرر في حادث مروري",
            primary_articles   = ("199",),     # المسؤولية التقصيرية (قانون مدني)
            required_elements  = (
                "وقوع خطأ من السائق المُتسبب",
                "حصول ضرر فعلي (مادي/بدني/معنوي)",
                "علاقة سببية بين الخطأ والضرر",
            ),
            common_defenses    = (
                "الخطأ من الطرف المدعي — لا مسؤولية على المدعى عليه",
                "الضرر غير ثابت أو مُبالغ فيه",
                "قوة قاهرة — لا تقصير",
                "رضا المضرور — تنازل ضمني",
            ),
            rebuttal_strategies = (
                "تقرير الخبير المروري",
                "صور موقع الحادث",
                "شهادة شهود العيان",
                "تقارير الطب الشرعي للإصابات",
                "فواتير المصاريف الطبية والصيانة",
            ),
        ),
    ),
    typical_counter_arguments = (
        "المخالفة مُثبتة برادار أو كاميرا — لا طعن",
        "السائق موقّع على المحضر — إقرار",
        "السحب يتماشى مع جسامة المخالفة",
    ),
    required_evidence = (
        "محضر الحادث / المخالفة",
        "قرار إداري بالسحب (إن كان موضوع الطعن)",
        "تقرير خبير مروري",
        "صور موقع الحادث",
        "تقارير طبية للإصابات",
        "شهادة شهود",
    ),
    competent_court    = "المحكمة المرورية (للمخالفات) / المحكمة المدنية (للتعويض)",
    appeal_court       = "محكمة الاستئناف — الدائرة المختصة",
    statute_of_limitations = (
        "دعوى المسؤولية التقصيرية (التعويض عن الحادث): ثلاث سنوات من "
        "علم المضرور بالضرر وبالشخص المسؤول عنه، وفق القواعد العامة "
    ...
    core_articles       = ("19", "66"),
    landmark_principles = (
        "تقدير الخطأ في الحوادث المرورية من سلطة محكمة الموضوع",
        "توفر السببية شرط لقيام المسؤولية",
        "قواعد المرور تُشكّل معياراً للسلوك الواجب",
        "سحب الرخصة إجراء إداري قابل للطعن",
        "التعويض يُقدّر بجميع عناصر الضرر (مادي + معنوي)",
    ),
    procedural_notes = (
        "التظلم من قرار سحب الرخصة: خلال 60 يوماً من التبليغ",
        "دعوى التعويض: تُرفع أمام المحكمة المدنية",
        "المخالفات البسيطة: تُدفع الغرامة مباشرة أو تُطعن عبر القسم المروري",
        "الحادث الجسيم: يحال إلى النيابة العامة كقضية جنائية",
    ),
    common_mistakes = (
        "دفع الغرامة ثم الطعن — يُعتبر إقراراً ضمنياً",
        "عدم طلب صور كاميرات المراقبة للحادث",
        "الاعتماد على شهادة ركاب المركبة فقط — شهادة متهمة",
    ),
)
